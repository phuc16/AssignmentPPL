...
mutable: VAR (nonvalue | value);
immutable: VAL (nonvalue | value);
...
nonvalue: (ID | DOLLARID) (COMMA nonvalue | COLON typ);
value: (ID | DOLLARID) (COMMA value COMMA exp | COLON typ ASSIOP exp);
...
paralist: (parasingle + SEMI)* parasingle;

parasingle: ID (COMMA ID)* COLON typ; 

methodattrdecl: (VAR | VAL) (nonvaluemethod | valuemethod) SEMI;

nonvaluemethod: ID (COMMA nonvaluemethod | COLON typ);

valuemethod: ID (COMMA valuemethod COMMA exp | COLON typ ASSIOP exp);
...
ifstmt: thenstmt elseifstmts elsestmt;

thenstmt: IF LB exp RB blockstmt;

elseifstmts: elseifstmt elseifstmts | ;

elseifstmt: ELSEIF LB exp RB blockstmt;
...
operands: literals | ID | LB exp RB | SELF | NULL;
